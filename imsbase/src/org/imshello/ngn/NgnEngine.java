...
import org.imshello.ngn.services.INgnHttpClientService;
...
import org.imshello.ngn.services.impl.NgnConfigurationService;
import org.imshello.ngn.services.impl.NgnContactService;
import org.imshello.ngn.services.impl.NgnHistoryService;
import org.imshello.ngn.services.impl.NgnHttpClientService;
import org.imshello.ngn.services.impl.NgnNetworkService;
import org.imshello.ngn.services.impl.NgnSipService;
import org.imshello.ngn.services.impl.NgnSoundService;
import org.imshello.ngn.services.impl.NgnStorageService;
import org.imshello.ngn.utils.NgnConfigurationEntry;
import org.imshello.baseWRAP.MediaSessionMgr;
import org.imshello.baseWRAP.ProxyAudioConsumer;
import org.imshello.baseWRAP.ProxyAudioProducer;
...
import org.imshello.baseWRAP.tmedia_mode_t;
...
import org.imshello.utils.AndroidUtils;
...
public class NgnEngine {
		...
		if(!sInitialized){
			...
			if (haveLibUtils) { // only "armeabi-v7a" comes with "libutils.so"
				...
				if(NgnApplication.isCpuNeon()){
					...
					System.load(String.format("%s/%s", NgnEngine.LIBS_FOLDER, "libbaseWRAP.so"));
					System.load(String.format("%s/%s", NgnEngine.LIBS_FOLDER, "libuvc_camera.so"));
				}
				else{
	...
	protected NgnEngine(){
		...
		MediaSessionMgr.defaultsSetVideoFps(getConfigurationService().getInt(NgnConfigurationEntry.QOS_VIDEO_FPS, NgnConfigurationEntry.DEFAULT_QOS_VIDEO_FPS));
