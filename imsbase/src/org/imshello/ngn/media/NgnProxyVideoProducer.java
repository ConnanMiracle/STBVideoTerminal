...
import org.imshello.utils.USBCamera.Size2;
import org.imshello.baseWRAP.ProxyVideoProducer;
import org.imshello.baseWRAP.ProxyVideoProducerCallback;
...
public class NgnProxyVideoProducer extends NgnProxyPlugin{
	...
	private MyProxyVideoProducerPreview2 mPreview2;
	...
	private ByteBuffer mVideoFrame,tmpBuffer;
	...
	public NgnProxyVideoProducer(BigInteger id, ProxyVideoProducer producer){
		...
		int usbCamStatus=NgnEngine	.getInstance().getConfigurationService().getInt(NgnConfigurationEntry.USB_CAMERA_STATUS,NgnConfigurationEntry.DEFAULT_USB_CAMERA_STATUS);
		useUSBCam=usbCamStatus==1?true:false;
    }
	...
	public final View startPreview(Context context){
		...
		if(((mPreview == null)&&(mPreview2 == null)) && mContext != null){
			if(!useUSBCam)
				mPreview = new MyProxyVideoProducerPreview(this);
			else
				mPreview2 = new MyProxyVideoProducerPreview2(this);
		}
		if((mPreview != null)||(mPreview2 != null)){
			if(!useUSBCam){
				...
				mPreview2.getHolder().setSizeFromLayout();
				mPreview2.bringToFront();
			}
		}
		if(!useUSBCam)
			return mPreview;
	...
	public void pushBlankPacket(){
		if(super.mValid && mProducer != null){
			if(mVideoFrame == null){
				mVideoFrame = ByteBuffer.allocateDirect((mWidth * mHeight * 4));
			}
		...
		try {
			if(super.mStarted){
				if (!useUSBCam) {
					...
					final USBCamera camera = NgnUSBCameraProducer.getCamera();
					if (pause) {
						camera.Stop();
					} else {
						camera.Start();
	...
	private synchronized int prepareCallback(int width, int height, int fps){
		...
		mFps = fps;
		Log.e(TAG, "FRAME INFO "+width+"*"+height+"  "+fps);
    ...
    private synchronized int startCallback(){
		...
		if(useUSBCam)
			if(mPreview2 != null)			
				startUSBCameraPreview(mPreview2.getCamera());    	
		return 0;
    }
    ...
    private synchronized int stopCallback(){
    	...
    	if(useUSBCam){
    		if(mPreview2 != null)
    			stopUSBCameraPreview(mPreview2.getCamera());
    	}
    ...
    private Size2 getUSBCameraBestPreviewSize(USBCamera camera){
    	final List<Size2> prevSizes = camera.getSupportedPreviewSizes();
    	
    	Size2 minSize = null;
    	int minScore = Integer.MAX_VALUE;
    	for(Size2 size : prevSizes){
    		/*final int score = Math.abs(size.width - mWidth) + Math.abs(size.height - mHeight);
    		if(minScore > score){
    			minScore = score;
    			minSize = size;
    ...
    private synchronized void startUSBCameraPreview(USBCamera camera){
    	if(!mStarted){
    		Log.w(TAG, "Someone requested to start camera preview but producer not ready ...delaying2");
    		return;
    	}
		if(camera != null && mProducer != null){
			try{				
				//Camera.Parameters parameters = camera.getParameters();
				//final Size2 prevSize = getUSBCameraBestPreviewSize(camera);
				//parameters.setPreviewSize(prevSize.width, prevSize.height);
				camera.setPrevSize2(mWidth,mHeight);
    ...
    private synchronized void stopUSBCameraPreview(USBCamera camera){
    	if(camera != null){
    		try{
    			camera.Stop();
    		}catch (Exception e) {
	...
	private MyUSBCameraCallback dataCallback = new MyUSBCameraCallback() {
		 
		@Override
		public void onFrame(byte[] data, int size) {
			// TODO Auto-generated method stub
			 if(mStarted){
				  if(NgnProxyVideoProducer.super.mValid && mVideoFrame != null && data != null){	
					  mVideoFrame.clear();
					 // Log.i(TAG, "size: "+size+" remaining "+mVideoFrame.remaining()+" capacity "+ mVideoFrame.capacity());
					  tmpBuffer=ByteBuffer.allocateDirect(size);
					  tmpBuffer.put(data, 0, size);
					  tmpBuffer.flip();
					  mProducer.push(tmpBuffer, size);
					  /*mVideoFrame.put(data,0,size);
					  mVideoFrame.flip();
					  mProducer.push(mVideoFrame, size);
					  mVideoFrame.rewind();*/
					}
			 }
		}
	...
	class MyProxyVideoProducerPreview2 extends SurfaceView implements SurfaceHolder.Callback {
		private SurfaceHolder mHolder;
		private final NgnProxyVideoProducer myProducer;
		private USBCamera mCamera;
	
		MyProxyVideoProducerPreview2(NgnProxyVideoProducer _producer) {
			super(_producer.mContext);
		...
		public void surfaceCreated(SurfaceHolder holder) {
			...
			try {
				...
				mCamera = NgnUSBCameraProducer.openCamera(myProducer.mFps, 
						myProducer.mWidth, 
						myProducer.mHeight, 
						this,
						myProducer.dataCallback
						);
			} catch (Exception exception) {
				Log.e(TAG, exception.toString());
			}
		...
		public void surfaceDestroyed(SurfaceHolder holder) {
			Log.d(TAG,"surfaceDestroyed()");
			try{
				NgnUSBCameraProducer.releaseCamera(mCamera);
			}
			catch (Exception exception) {
				Log.e(TAG, exception.toString());
			}
		...
		public void surfaceChanged(SurfaceHolder holder, int format, int w, int h) {
			...
			try{
				if(mCamera != null){
					myProducer.startUSBCameraPreview(mCamera);
				}
			}
			catch (Exception exception) {
				Log.e(TAG, exception.toString());
