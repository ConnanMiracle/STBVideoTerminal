...
public class NgnProxyVideoConsumerGL extends NgnProxyVideoConsumer{
	...
	private ByteBuffer BufferY,BufferU,BufferV;
    	...
    	synchronized(this){
	    	...
	    	if(mContext != null){
		    	if(mPreview == null || mPreview.isDestroyed()){
		    		...
		    		mPreview = new NgnProxyVideoConsumerGLPreview(mContext, super.mFullScreenRequired,  null, mWidth, mHeight, mFps);
		    		mPreview.setBuffer(BufferY,BufferU,BufferV, mWidth, mHeight);
		    		Log.e(TAG, "start preview: "+String.valueOf(mPreview==null)+mWidth+mHeight);
		    	}
	    	}
			return mPreview;
    	}
    ...
    private int prepareCallback(int width, int height, int fps){
    	synchronized(this){
			...
			//mDeocodedFrame = ByteBuffer.allocateDirect((mWidth * mHeight * 3) >> 1);
			BufferY=ByteBuffer.allocateDirect(mWidth * mHeight);
			BufferU=ByteBuffer.allocateDirect(mWidth * mHeight/4);
			BufferV=ByteBuffer.allocateDirect(mWidth * mHeight/4);
			...
			if(isH264passThrough){
				...
				//decoder.setDeocdeBuffer(mDeocodedFrame);
				decoder.setYUVBuffer(BufferY, BufferU, BufferV);
			}
    ...
    private int bufferCopiedCallback(long nCopiedSize, long nAvailableSize) {
		...
		//TODO check display width . the following code reset mWidth mHeight may lead to error
		//if(mVideoFrame == null || mWidth != frameWidth || frameHeight != mHeight || mVideoFrame.capacity() != nAvailableSize){
		/*if(mPreview.mBufferY==null  ||mPreview.mBufferU == null ||mPreview.mBufferV == null || mWidth != frameWidth || frameHeight != mHeight){
			Log.e(TAG, " "+mWidth+" "+frameWidth+" "+frameHeight+" "+mHeight+" "+mVideoFrame.capacity()+" "+nAvailableSize);
		...
		if(isH264passThrough){
			...
			/*ByteBuffer ret=decoder.decode2(mVideoFrame, (int)nCopiedSize,(int)nAvailableSize);
			mPreview.setBuffer(ret, mWidth, mHeight);
			Log.i(TAG, "decoded return "+ret);
			if(ret!=null)
				mPreview.requestRender();*/
			return 0;
	...
	static class NgnProxyVideoConsumerGLPreview extends GLSurfaceView implements GLSurfaceView.Renderer{
		...
		ByteBuffer mBuffer;
		ByteBuffer mBufferY,mBufferU,mBufferV;
		...
		public void setBuffer(ByteBuffer bufferY,ByteBuffer bufferU,ByteBuffer bufferV, int bufferWidth, int bufferHeight){
			mBufferY=bufferY;
			mBufferV=bufferV;
			mBufferU=bufferU;
			...
			mBufferWidthUV = (mBufferWidthY >> 1);
			mBufferHeightUV = (mBufferHeightY >> 1);
			Log.e(TAG, "set YUV GL buffer "+String.valueOf(mBufferY==null)+String.valueOf(mBufferU==null)+String.valueOf(mBufferV==null));
		...
		public void onDrawFrame(GL10 glUnused) {
	        ...
	        //Log.e(TAG, "GL: "+this.mBufferWidthY+this.mBufferHeightY);
	        //if(mBuffer != null){
	        if((mBufferY != null)&&(mBufferU != null)&&(mBufferV != null)){
	        	//Log.i(TAG, "GL: drawing"+mBufferWidthY+mBufferHeightY+ mBufferWidthUV+ mBufferHeightUV);
	        	synchronized(this){
			        ...
			        GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_LUMINANCE, mBufferWidthY, mBufferHeightY, 0, GLES20.GL_LUMINANCE, GLES20.GL_UNSIGNED_BYTE, mBufferY);
			        ...
			        GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_LUMINANCE, mBufferWidthUV, mBufferHeightUV, 0, GLES20.GL_LUMINANCE, GLES20.GL_UNSIGNED_BYTE, mBufferU);
			        ...
			        GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_LUMINANCE, mBufferWidthUV, mBufferHeightUV, 0, GLES20.GL_LUMINANCE, GLES20.GL_UNSIGNED_BYTE, mBufferV);
