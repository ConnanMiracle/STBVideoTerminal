...
import org.imshello.baseWRAP.ProxyAudioProducerCallback;
...
public class NgnProxyAudioProducer extends NgnProxyPlugin {
	...
	private ByteBuffer blankPacketBuffer;//added by wangy
	...
	//added by wangy for sending blank packet to open NAT 20141112 
	public void pushBlankPacket(){
		     //发送的缓冲区
			 ByteBuffer mAudioBlankFrame;
			 //缓冲区大小
			 mAudioBlankFrame = ByteBuffer.allocateDirect(32);
			 int nSize = mAudioBlankFrame.capacity();
			 byte pktNAT[] = new byte[nSize];
			 //添加标记
			 pktNAT[0] = 'N';
			 pktNAT[1] = 'A';
			 pktNAT[2] = 'T';
			 //缓冲区赋值（接收端测试）
			 mAudioBlankFrame.put(pktNAT);
			 Log.d(TAG, "空包生成了");
			 Log.d(TAG, mAudioBlankFrame.toString());
			 if(super.mValid && mProducer != null)
			 {
				 //将NAT数据包添加到发送队列
			     mProducer.push(mAudioBlankFrame, 32000000);
			     Log.d(TAG, "空包发送了");
			 }
			 mAudioBlankFrame.clear();
	}
