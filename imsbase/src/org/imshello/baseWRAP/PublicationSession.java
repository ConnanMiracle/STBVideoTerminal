/* ----------------------------------------------------------------------------
 * This file was automatically generated by SWIG (http://www.swig.org).
 * Version 3.0.0
 *
 * Do not make changes to this file unless you know what you are doing--modify
 * the SWIG interface file instead.
 * ----------------------------------------------------------------------------- */

package org.imshello.baseWRAP;

public class PublicationSession extends SipSession {
  private long swigCPtr;

  protected PublicationSession(long cPtr, boolean cMemoryOwn) {
    super(baseWRAPJNI.PublicationSession_SWIGUpcast(cPtr), cMemoryOwn);
    swigCPtr = cPtr;
  }

  protected static long getCPtr(PublicationSession obj) {
    return (obj == null) ? 0 : obj.swigCPtr;
  }

  protected void finalize() {
    delete();
  }

  public synchronized void delete() {
    if (swigCPtr != 0) {
      if (swigCMemOwn) {
        swigCMemOwn = false;
        baseWRAPJNI.delete_PublicationSession(swigCPtr);
      }
      swigCPtr = 0;
    }
    super.delete();
  }

  public boolean Publish(byte[] bytes) {
    if(bytes != null){
		final java.nio.ByteBuffer byteBuffer = this.getByteBuffer(bytes);
        return this.publish(byteBuffer, bytes.length);
    }
    return false;
  }

  public PublicationSession(SipStack pStack) {
    this(baseWRAPJNI.new_PublicationSession(SipStack.getCPtr(pStack), pStack), true);
  }

  public boolean publish(java.nio.ByteBuffer payload, long len, ActionConfig config) {
    return baseWRAPJNI.PublicationSession_publish__SWIG_0(swigCPtr, this, payload, len, ActionConfig.getCPtr(config), config);
  }

  public boolean publish(java.nio.ByteBuffer payload, long len) {
    return baseWRAPJNI.PublicationSession_publish__SWIG_1(swigCPtr, this, payload, len);
  }

  public boolean unPublish(ActionConfig config) {
    return baseWRAPJNI.PublicationSession_unPublish__SWIG_0(swigCPtr, this, ActionConfig.getCPtr(config), config);
  }

  public boolean unPublish() {
    return baseWRAPJNI.PublicationSession_unPublish__SWIG_1(swigCPtr, this);
  }

}
